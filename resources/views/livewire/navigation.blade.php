<nav class="bg-gray-800" x-data='{open:false}'>
    ...
    <!-- Mobile menu button-->
      <div  class="absolute inset-y-0 left-0 flex items-center sm:hidden">
        <button  type="button" class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white" aria-controls="mobile-menu" aria-expanded="false">
          ...
          -->
          <svg x-on:click='open=true' id="botonMenu" class="hidden h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
          ...
          -->
          <svg x-on:click='open=false' id="botonX" class="hidden h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
  ...
  <div x-show="open" x-on:click='open=false' class="sm:hidden" id="mobile-menu">
...
//se le da un ID a cada imagen del boton y se crean las contantes
const botonMenu = document.getElementById('botonMenu'); 
const botonX = document.getElementById('botonX');

//como la imagen se oculta automaticamente
//aqui hacemos que cambie automaticamente a BLOCK
botonMenu.classList.toggle('hidden');

//funcion que detecta cuando le damos click a la imagen HAMBURGUESA
//para que cambie de estado de hidden a block
botonMenu.addEventListener('click', ()=>{
  botonX.classList.toggle('hidden');
  botonMenu.classList.toggle('hidden');
})

//funcion que detecta cuando le damos click a la imagen X
//para que cambie de estado de hidden a block
